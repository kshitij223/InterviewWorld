...
public class ImmutableQueue_orig<E>{//ImmutableQueue class
	...
	public ImmutableQueue_orig(ImmutableStack<E> order,ImmutableStack<E> reverse){
		this.order = order;
	...
	public ImmutableQueue_orig<E> enqueue(E object){
		if(object==null)
			throw new IllegalArgumentException();
		return new ImmutableQueue_orig<E>(this.order.push(object),this.reverse);
	}
	...
	public ImmutableQueue_orig<E> dequeue(){
		if(this.isEmpty())
			throw new NoSuchElementException();
		if(!this.reverse.isEmpty()){
			return new ImmutableQueue_orig<E>(this.order,this.reverse.tail);
		}else{
			return new ImmutableQueue_orig<E>(ImmutableStack.emptyStack(),this.order.toReverseStack().tail);
		}		
	}
